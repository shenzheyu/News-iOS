...
class SearchResultController: UITableViewController {
    ...
    func requestResults() {
        Alamofire.request("https://csci571-homework9-news-api.ue.r.appspot.com/search/\(keyword)").responseJSON{ response in
            if let json = response.result.value {
                let articlesJSON = JSON(json)["articles"]
                self.createResults(articlesJSON: articlesJSON)
                self.tableView.reloadData()
                SwiftSpinner.hide()
            }
        }
    ...
    func createResults(articlesJSON: JSON) {
        articles.removeAll()
...
extension SearchResultController: addBookmarkDelegate {
